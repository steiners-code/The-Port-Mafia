...
import { handleStrategistResponse } from "./handleStrategistResponse";
import { resolveStrategistNeeds } from "./resolveStrategistNeeds";
...
export type StrategistFinalResponse = {
    title: string;
...
export type StrategistResponse = StrategistNeedsResponse | StrategistFinalResponse;

function isFinalResponse(parsed: StrategistResponse): parsed is StrategistFinalResponse {
    return "title" in parsed;
}
...
export async function generateStrategistResponse({ messageId, userId, principalName, schema, category }: GenerateStrategistResponse) {
    ...
    try {
            ...
            for await (const event of stream) {
                ...
                switch (event.event_type) {
                    ...
                    case "step.stop":
                        ...
                        // This model turn's content is now persisted —
                        // record it in B's own history bucket so a needs
                        // turn can be resumed, or so the eventual final
                        // turn has this turn as prior context.
        ...
        console.error("[generateStrategistResponse] fatal error:", error);
